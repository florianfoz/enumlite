...
struct qt_backend {
  ...
  static string_type substr(const string_type &value, size_type pos,
                            size_type count = npos) {
    return value.mid(pos, count);
  }

  static size_type find(const string_type &value, char_type c,
                        size_type pos = 0) noexcept {
    return value.indexOf(c, pos);
  }

  static void erase(string_type &value, size_type pos,
  ...
  static void append(string_type &value, char_type c) { value += c; }

  static char_type to_lower(char_type c) noexcept { return c.toLower(); }
