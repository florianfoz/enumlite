...
static void test_flags_underlying() {
  ...
  static_assert(
      std::is_same_v<std::underlying_type_t<Features>, unsigned char>);
}

static void test_flags_all() {
  constexpr auto all = static_cast<unsigned>(Permissions::ALL);

  constexpr auto expected = static_cast<unsigned>(Permissions::READ) |
                            static_cast<unsigned>(Permissions::WRITE) |
...
static void test_flags_operators() {
  ...
  assert(static_cast<unsigned>(inverted) ==
         ~static_cast<unsigned>(Permissions::NONE));
}
...
static void test_flags_to_string() {
  ...
  assert(Permissions_to_str(Permissions::READ | Permissions::WRITE) ==
         "READ|WRITE");

  assert(Permissions_to_str(Permissions::READ | Permissions::WRITE |
                            Permissions::EXECUTE) == "READ|WRITE|EXECUTE");
...
static void test_flags_to_vec_string() {
  ...
  {
    const auto result =
        Permissions_to_vec_str(Permissions::READ | Permissions::WRITE);
...
static void test_flags_from_string() {
  ...
  assert(Permissions_from_str("READ|WRITE") ==
  ...
  assert(Permissions_from_str("_READ_|_WRITE_") ==
         (Permissions::READ | Permissions::WRITE));
...
static void test_small_underlying_flags() {
  assert(static_cast<unsigned char>(Features::ALL) ==
